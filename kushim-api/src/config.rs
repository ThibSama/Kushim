...
impl Config {
    pub fn from_env() -> Result<Self> {
        let database_url =
            env::var("DATABASE_URL").context("DATABASE_URL must be set for kushim-api")?;
        let redis_url = env::var("REDIS_URL").ok();
        let host = env::var("KUSHIM_API_HOST").unwrap_or_else(|_| "0.0.0.0".to_string());
        let port = env::var("KUSHIM_API_PORT")
            .unwrap_or_else(|_| "8080".to_string())
            .parse::<u16>()
            .context("KUSHIM_API_PORT must be a valid u16")?;
        ...
        {
            bail!("REDIS_URL must not be blank when provided");
        }
    ...
    pub fn socket_addr(&self) -> Result<SocketAddr> {
        format!("{}:{}", self.host, self.port)
            .parse()
            .context("KUSHIM_API_HOST and KUSHIM_API_PORT must produce a valid socket address")
    }
}
