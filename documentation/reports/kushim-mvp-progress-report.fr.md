...
- le backend E2E est désormais démontrable localement via un smoke test automatisé (`scripts/powershell/demo/backend-e2e.ps1`, 18/18 assertions passées) ;
...
- le catalogue d'actifs MVP dispose d'un seed canonique stable (`infra/postgres/init/002_seed_canonical_assets.sql`) pour AAPL, MSFT et NVDA ; les démos backend, les validations Finnhub contrôlées et les tests d'intégration market-data réutilisent ces lignes au lieu d'en créer à chaque exécution ;
- une base de données neuve supporte le signup sans insertion SQL manuelle : le rôle `user` (données de référence, `id_role = 1`, aucune donnée d'identité) est seedé par `infra/postgres/init/003_seed_auth_roles.sql` ; le backend E2E valide désormais `DemoPrefix` localement avant l'appel signup ; un job CI dédié `fresh-db-bootstrap` valide l'amorçage complet (schéma + seed actifs + seed rôle) de façon idempotente ;
- rafraîchissement automatique du portefeuille (P0) : poser une opération met en file une requête durable `portfolio_refresh_requests` dans la même transaction PostgreSQL ; `kushim-worker` (mode boucle, job `process_portfolio_refresh_requests`) la consomme automatiquement (`FOR UPDATE SKIP LOCKED`, retry borné, récupération de verrou périmé) et reconstruit read models courants + snapshot quotidien courant ; le frontend suit la requête via `GET /v1/portfolios/{id}/refresh-requests/{id}` et recharge les données réelles à la complétion. Plus aucune commande worker manuelle après une opération. Validé E2E (18 assertions, sans rebuild/snapshot manuel) ;
...
- **smoke test E2E automatisé : 18/18 assertions passées** (`scripts/powershell/demo/backend-e2e.ps1`)
...
Le smoke test backend E2E est implémenté et validé : `scripts/powershell/demo/backend-e2e.ps1` (18/18 assertions).
